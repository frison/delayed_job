...
By allowing a DJ handler to do some preprocessing before the DJ table is locked, we can create recurring events using the following code:
<pre><code>
  class RecurringEvent

    def preprocess
      recur = self.class.new

      #Don't want to create another one if there is already 2 going in the next 2 minutes
      if (Delayed::Job.find_all_by_handler(recur.to_yaml, :conditions => ['run_at <= ?', Time.now + 2.minutes]).length <= 1)
        Delayed::Job.create({:run_at => Time.now + 37.seconds, :handler => recur }) # This will run again in 37 seconds
      end

    end

    def perform
      Reminder.reminder_everyone_too_often
    end
