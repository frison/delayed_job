...
h2.
  ...
  By allowing a DJ handler to do some preprocessing before the DJ table is locked, we can create recurring events using the
  following code:
<pre><code>
  class RecurringEvent
    def preprocess
      ...
      if (Delayed::Job.find_all_by_handler(recur.to_yaml, :conditions => ['run_at <= ?', Time.now + 2.minutes]).length <= 1)
        Delayed::Job.create({:run_at => Time.now + 37.seconds, :handler => me }) # This will run again in 37 seconds
      end
    end
    def perform
